...
void CANSend(CAN_msg_t* CAN_tx_msg)
{
  ...
  if (CAN1->sTxMailBox[0].TIR & 0x1UL) {
    ...
    Serial.println("ESR:");
    Serial.print("REC :");
    Serial.print(CAN1->ESR >> 24);
    Serial.print("  TEC :");
    Serial.print((CAN1->ESR >> 16) & 0xFF);
    Serial.print("  LEC :");
    Serial.print((CAN1->ESR >> 6) & 0b1);
    Serial.print((CAN1->ESR >> 5) & 0b1);
    Serial.println((CAN1->ESR >> 4) & 0b1);
...
void setup() {
  ...
  bool ret = CANInit(CAN_50KBPS, 0);  // CAN_RX mapped to PA11, CAN_TX mapped to PA12
...
void loop() {
 ...
 // Serial.println(CANMsgAvail());
  if(CANMsgAvail()) {
    //Serial.println("Je suis dans le if");
    CANReceive(&CAN_RX_msg);
    unsigned int myId = CAN_RX_msg.id; // Récupération de la valeur de CAN_RX_msg.id
char myIdStr[5]; // Tableau de caractères pour stocker la chaîne de caractères formatée

// Formatage de la valeur hexadécimale en une chaîne de caractères
sprintf(myIdStr, "%0X", myId);

// Affichage de la chaîne de caractères via la console série

int myIdInt = strtol(myIdStr, NULL, 16);
if(strcmp(myIdStr,"126")== 0){
  
    Serial.print("Je viens de recevoir un message qui était destiné à moi :");
    Serial.print(F("\nID dest 0x"));Serial.println(CAN_RX_msg.id,HEX);
    for(int i = 0; i< CAN_RX_msg.len; i++){
        //Serial.println(CAN_RX_msg.data[i]); 
        Serial.print(F("0x"));Serial.println(CAN_RX_msg.data[i],HEX);
    }
  
}
else{
  Serial.print("Le message avec l'id :");Serial.print(F("\nID dest 0x"));Serial.print(CAN_RX_msg.id,HEX);
  Serial.println(" ne m'est pas destiné");
}
